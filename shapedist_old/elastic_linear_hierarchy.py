...
def find_gamma(t, p, q, parametrization_array, energy_dot):
    ...
    while True:
        ...
        i = n-1
        j = m-1
        min_energy_values[i][j] = integrate(tp_temp, tq_temp, py_temp, qy_temp, 0, i, 0, j, gamma_interval, energy_dot)
        k = i - neighborhood
        if k <= 0:
            k = 1
        minimum = min_energy_values[i][j]
        ...
        # plt.plot(tg[parametrization_array[current_iteration]][:n], path[:n], ".-b")
        # plt.plot(tg[parametrization_array[current_iteration]][:n], path[:n] + strip_height * gamma_interval, "-r")
        # plt.plot(tg[parametrization_array[current_iteration]][:n], path[:n] - strip_height * gamma_interval, "-r")
