...
for i in text:
    ...
    domain_x, gammax, val = elastic_n_2.find_gamma(np.array([t, p]), np.array([t, q]), np.array([tg, gamma]),  m // 2, m // 2)
    ...
    for j in range(5):
        cProfile.run("elastic_n_2.find_gamma(np.array([t, p]), np.array([t, q]), np.array([tg, gamma]),  m // 4, m // 4)", 'statsfile')
        stream = StringIO()
